...
def read_incoming_serial(sp, callbacks):
    while True:
        try:
            ...
            print_with_timestamp(f"OS error occurred: {e}")
            break
        except Exception as e:
            print_with_timestamp(f"Unexpected error occurred: {e}")
            break
...
def run(callbacks):
    ...
    try:
        if _sp and _sp.is_open:
            print_with_timestamp("Starting serial read thread...")
            ...
            print_with_timestamp("Serial port not open or invalid.")
    except serial.SerialException as e:
        print_with_timestamp(f"Serial error occurred: {e}")
    finally:
        if _sp and _sp.is_open:
            print_with_timestamp("Closing serial port...")
            _sp.close()
            print_with_timestamp("Serial port closed.")
